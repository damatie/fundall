...
const OnboardingStepperMain = () => {
	...
	function getStepContent(stepIndex) {
		switch (stepIndex) {
			...
			case 1:
				return <SpouseAndDependant goToNextStepper={handleNext}/>;
			case 2:
				return <NextOfKin />;
			case 3:
		...
		<PageLayout
			...
			content={
				<div className={classes.onBoardingStepperMain}>
					<Stepper activeStep={activeStep} alternativeLabel className={` ${classes.stepperBg} ${classes.stepperPointer}`}>
						{steps.map((label, index) => (
							<Step key={label} onClick={(ev) => handleStepperClick(index)} className={`${classes.stepperPointer}`}>
