...
export const GET_ALL_KPOS_BY_LINE_MANAGER = 'GET_ALL_KPOS_BY_LINE_MANAGER';
...
export const getKpoByDept = id => {
	return async dispatch => {
		try {
			...
			} = await api.get(`/appraisal/kpo/dept/${id}`);
			if (success) {
				dispatch({
...
export const getAssignedKpo = () => {
	return async dispatch => {
		try {
			...
			} = await api.get(`/appraisal/kpo/get-review/`);
			if (success) {
				dispatch({
...
export const getKpoByEntity = id => {
	return async dispatch => {
		try {
			...
			} = await api.get(`/appraisal/kpo/entity/${id}`);
			dispatch({
				type: GET_KPO_BY_DEPT,
				payload: getApprovedKpo(rows)
			});
...
export const getKpoByStatus = ({ status, requested }) => {
	return async dispatch => {
		try {
			...
			} = await api.get(`/appraisal/kpo/?status=${status}`);
			if (requested) {
				dispatch({
...
export const kpoReq = ({ id, type }) => {
	return async dispatch => {
		try {
			...
			const {
				data: { message }
			} = await api.patch(`/appraisal/kpo/create/request/${type}/${id}`);
			swal.fire({
				text: message,
				icon: 'success'
			});
