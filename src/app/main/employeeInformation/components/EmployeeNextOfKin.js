...
import { getNextOfKin } from '../store/actions';
...
const EmployeeNextOfKin = ({ handleOpen }) => {
  ...
  React.useEffect(() => {
    dispatch(getNextOfKin(getUserId));
          ...
          <>
            {
              canAdd() && (<SharedButton
                variant='outlined'
                color='secondary'
                onClick={handleOpen('Employee Next of Kin')}
                className='mx-16'
              >
                Add
              </SharedButton>)
...
const EmployeeNextOfKinDetails = ({ item, index, setShouldUpdate, shouldUpdate, canDelete }) => {
  ...
  return (
    <>
      <div className='flex flex-row items-center my-20'>
        <Typography variant="subtitle1" color="initial">Employee Next of Kin ({index + 1})</Typography>
